	...
	$.ajax({
		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				'<td>'+
					...
					'<select class="form-control" name="productName[]" id="productName'+count+'" onchange="getProductData('+count+')" >'+
						...
						for (var i=0;i<response.results.length;i++){
                            // $('<option/>').val(data.results[i]).html(data.results[i]).appendTo('#ak');
                            tr += '<option value="'+response.results[i].id+'">'+response.results[i].pname+'</option>';	
                        }


						// $.each(response, function(index, value) {
						// 	tr += '<option value="'+value.id+'">'+value.pname+'</option>';							
						// });
				...
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;"">'+
					'<input type="text" name="qrate[]" id="qrate'+count+'" autocomplete="off" disabled="true" class="form-control" />'+
					'<input type="hidden" name="qrateValue[]" id="qrateValue'+count+'" autocomplete="off" class="form-control" />'+
				'</td style="padding-left:20px;">'+
				'<td style="padding-left:20px;">'+
					'<div class="form-group">'+
...
function getProductData(row = null) {
	...
	if(row) {
		...
		if(productId == "") {
			...
			// alert("rate-->"+row);
			$.ajax({
		url: '/staff/fetchSelectedProduct/',
		// type: 'post',
		data: {productId : productId},
		dataType: 'json',
		success:function(response) {
			 // alert(response.rslt[0].price)
		
			 		$("#rate"+row).val(response.rslt[0].price);
					$("#rateValue"+row).val(response.price);
					
					$("#qrate"+row).val(response.rslt[0].quantity);
					$("#qrateValue"+row).val(response.quantity);
...
function getTotal(row = null) {
	if(row) {
		...
		var c=Number($("#qrate"+row).val());
		var b=$("#quantity"+row).val();
